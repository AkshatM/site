...
This is because `std::time` is not supported by the `wasm32-unknown-unknown` compile target. Oh, it'll _compile_ fine - the problem is it'll throw [a runtime panic](https://internals.rust-lang.org/t/is-std-instant-on-webassembly-possible/18913/2) whenever `time` functions are invoked! 
